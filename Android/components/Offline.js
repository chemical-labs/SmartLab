...
import { View, TouchableOpacity, CameraRoll, Text, Switch, Image, TextInput, FlatList, AsyncStorage, ScrollView, RefreshControl, Button, Picker, AppRegistry, ImageBackground, StatusBar } from 'react-native'
                        ...
                        <View style={{ backgroundColor: 'white', padding: 15, borderRadius: 10, }}>
                            ...
                            <View style={{ alignItems: 'center', marginTop: 5 }}>
                                ...
                                <View style={{ alignItems: 'center' }}>
                                    <Text>Aplikasi di update ke v1.2.8</Text>
                                    <Text>Jadi, apa yang baru ?</Text>

                                    <View style={{ marginTop: 10, alignItems: 'center' }}>
                                        <Text style={{ fontWeight: 'bold' }}>Update Fitur Login</Text>
                                        <Text style={{ fontWeight: 'bold' }}>Update Fitur Tambah Users</Text>
                                    </View>
                                </View>
                            </View>
                        </View>
                ...
                <Modal isVisible={this.state.nomachine_alert}>
                    <View style={{ flex: 1, flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                        <View style={{ backgroundColor: 'white', padding: 10, borderRadius: 10, alignItems: 'center', paddingLeft: 12, paddingRight: 12 }}>
                            ...
                            <Text style={{ fontWeight: 'bold', fontSize: 16 }}>Mesin Tidak Ada !</Text>
                            <Image source={require('../assets/illustrations/offline.png')} style={{ width: 130, height: 120, marginTop: 10 }} />
                            <Text>Aplikasi Belum Terkoneksi</Text>
                            <Text>Ke mesin IOT </Text>
                            <Text>Silahkan tambahkan mesin</Text>
                            <Text>Dengan menekan tombol di bawah</Text>
                            ...
                            <TouchableOpacity style={{ marginTop: 15, padding: 5, borderRadius: 5, backgroundColor: 'black', elevation: 15 }} onPress={() => this.props.navigation.navigate('ProfileOffline')}>
                                <Text style={{ color: 'white', fontWeight: 'bold' }}>Tambah Mesin</Text>
                            </TouchableOpacity>
                        </View>
                    </View>
                </Modal>
                ...
                <Modal isVisible={this.state.authors}>
                    <View style={{ flex: 1, flexDirection: 'column', justifyContent: 'center', alignItems: 'center' }}>
                        <View style={{ backgroundColor: 'white', padding: 10, borderRadius: 5, alignItems: 'center', }}>
                            <Text style={{ fontWeight: 'bold', fontSize: 17 }}>Developer</Text>
                ...
                <ScrollView style={{ marginTop: 35 }}>
                    <View style={{ flexDirection: 'column', justifyContent: 'space-between' }}>
                        ...
                        <View style={{ marginTop: 250 }}>
                            <TouchableOpacity style={{ backgroundColor: 'red', elevation: 15 }} onPress={() => this.logout()}>
        ...
        AsyncStorage.getItem('localip').then(localip => {
            (async() => {
                ...
                if(parse.type_sensor == 'lock.png'){
                    ...
                    setInterval(() => {
                        axios.post('http://' + parse.machine + parse.url + 'die').then(data => {
                            if(data.status == 200){
                                this.setState({ loading: false })
                                this.setState({ lock_delay: false })
    ...
    render(){
        return(
            <BarCodeScanner onBarCodeScanned={ this.state.scan ? undefined : ({ type, data }) => {
                ...
                <Modal isVisible={this.state.lock_delay}>
                    <View style={{ flex: 1, flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                        <View style={{ padding: 10, backgroundColor: 'white', borderRadius: 10, alignItems: 'center' }}>
                            <Text style={{ fontWeight: 'bold', fontSize: 16 }}>Opening The Door</Text>
                            <Image source={require('../assets/illustrations/login.png')} style={{ width: 150, height: 100, marginTop: 13 }} />
                            <Text style={{ marginTop: 5 }}>Pintu Terbuka</Text>
                            <Text>Selama 10 Detik..</Text>
                        </View>
                    </View>
                </Modal>
...
class HomePage extends Component{
    ...
    async componentDidMount(){
        ...
        AsyncStorage.getItem('token').then(data => {
            axios.post(konfigurasi.server + 'auth/getall', { token: data, secret: konfigurasi.key }).then(response => {
                if(response.status == 200){
                    this.setState({ name: response.data.result.username })
                }
            })
        })
        ...
        AsyncStorage.getItem('serial_offline').then(data => {
            ...
            axios.get('http://wttr.in/?format=j1').then(res => {
                ...
                if(this.state.weatherStatus.match(/Thunder/i)){
                    this.setState({ weather: require('../assets/weather/thunder.png'), weatherCondition: "Hujan", weatherPallete: 'black', weatherFont: 'white' })
                    ...
                    this.setState({ weather: require('../assets/weather/cloudy.png'), weatherCondition: 'Berawan Cerah', weatherPallete: 'white', weatherFont: 'black' })
                }else if(this.state.weatherStatus.match(/Rain/i)){
                    this.setState({ weather: require('../assets/weather/rain.png'), weatherCondition: 'Gerimis', weatherPallete: 'grey', weatherFont: 'white' })
    ...
    async refresh(){
        ...
        AsyncStorage.getItem('token').then(data => {
            axios.post(konfigurasi.server + 'auth/getall', { token: data, secret: konfigurasi.secret }).then(response => {
                if(response.status == 200){
                    this.setState({ name: response.result.username })
                }
            })
        })
    ...
    unlock(index, url, status, machineIP){
        (async() => {
            this.setState({ loading: true })
            await axios.post('http://' + machineIP + url).then(response => {
                this.notif('Opening The Door', 'Hei ' + this.state.name + ', Pintu Terbuka selama 10 detik')
                this.setState({ lock_delay: true })
            }).catch(err => {
                this.setState({ loading: false, error_server: true })
            })
            ...
            setTimeout(() => {
                axios.post('http://' + machineIP + url + 'die').then(response => {
                    this.setState({ lock_delay: false })
                }).catch(err => {
                    this.setState({ loading: false, error_server: true })
                })
            ...
            <ScrollView contentContainerStyle={{ flexGrow: 1, flexDirection: 'column', alignItems: 'center', backgroundColor: '#292928' }} refreshControl={<RefreshControl refreshing={this.state.refresh} onRefresh={() => this.refresh()}/>}>
                    ...
                    <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
                        <View style={{ backgroundColor: 'white', padding: 15, borderRadius: 15 }}>
                            ...
                            <View style={{ marginTop: 15, alignItems: 'center' }}>
                                ...
                                <Picker selectedValue={this.state.newRelay} onValueChange={(val) => this.setState({ newRelay: val })} style={{ marginLeft: -8, width: 120, height: 50 }}>
                                ...
                                <ScrollView style={{ flexGrow: 1, flexDirection: 'column' }}>
                                    ...
                                    return <TouchableOpacity style={{ flexDirection: "row", backgroundColor: 'black', justifyContent: 'space-between', padding: 20, width: 280, marginTop: 19, borderRadius: 10 }} onPress={() => this.moduleDetail(x.name, x.uri_on, x.uri_off, y, x.machineIP)}>
                                        ...
                                        <View style={{ marginLeft: 50, marginTop: 12 }}>
                                            {x.type == 'lock.png' ? <TouchableOpacity style={{ marginRight: 10 }} onPress={() => this.unlock(y, x.uri_on, x.relay_status, x.machineIP)}>
                                                <Icon name='lock-open-outline' size={30} color="white" />
                                ...
                                <View style={{ marginLeft: 20 }}>
                                    <View style={{ alignItems: 'center' }}>
                                        <TouchableOpacity onPress={() => this.register()}>
                                            <Image source={require('../assets/icons/users.png')} style={{ width: 70, height: 70, alignSelf: 'center' }} />
                                            <Text style={{ fontWeight: 'bold', color: 'orange', textAlign: 'center' }}>Tambah Users</Text>
                                        </TouchableOpacity>
                                    </View>
                                </View>
                            ...
                            <View style={{ flexDirection: 'row', marginTop: 25, justifyContent: 'space-between' }}>
                                <View style={{ flexDirection: 'column', marginLeft: -10 }}>
                                    ...
                                    <Picker selectedValue={this.state.relay_category} onValueChange={(val) => this.setState({ relay_category: val })} style={{ marginLeft: -8, width: 110, height: 50 }}>
                                        <Picker.Item label="Electro" value="lights.png" />
                                        <Picker.Item label="Lock" value="lock.png" />
                                    </Picker>

                                    {this.state.machine.length == 0 ? <Text style={{ color: 'red', fontWeight: 'bold' }}>No Machine!</Text> : <Picker selectedValue={this.state.machineIP} onValueChange={(val) => this.setState({ machineIP: val })} style={{ marginLeft: -8, width: 110, height: 50 }}>
                                        {this.state.machine.map((x, y) => {
                                        return <Picker.Item label={x.name} value={x.ip} />
                                        })}
                                    </Picker>}
                                    ...
                                    <Picker selectedValue={this.state.uri_on} onValueChange={(val) => this.setState({ uri_on: val })} style={{ marginLeft: -8, width: 120, height: 50 }}>
                                        <Picker.Item label="Relay 1" value="relay1" />
                                        <Picker.Item label="Relay 2" value="relay2" />
                                        <Picker.Item label="Relay 3" value="relay3" />
                                        <Picker.Item label="Relay 4" value="relay4" />
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 45 }}>
                    <View style={{ flexDirection: 'column' }}>
                        ...
                        <Text style={{ color: '#ededed' }}>{this.state.waktu} {this.state.name.length > 5 ? 'Users' : this.state.name}</Text>
